...
try:
    ...
    print("✓ PaddleOCR 和 pdf2image 导入成功")
except ImportError as e:
    print(f"错误: 无法导入所需库: {e}")
    print("请确保在paddleocr环境中运行: conda activate paddleocr")
    sys.exit(1)
...
def extract_text_from_pdf(pdf_path, output_md_path):
    ...
    try:
        ...
        # OCR识别
        full_text = []
        full_text.append(f"# {os.path.basename(pdf_path).replace('.pdf', '')}\n")
...
def main():
    ...
    print(f"平均每个文件: {total_time/stats['total']/60:.1f} 分钟")
    print("=" * 60)
