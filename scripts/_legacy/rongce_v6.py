...
WK = Path("work/sichuan_rongce_brochure/v6_assets")
DOCX = OUT / "四川融策宣传册_完善稿_v6_视觉升级版.docx"
PDF = OUT / "四川融策宣传册_完善稿_v6_视觉升级版.pdf"

NW = "#1A365D"; TL = "#3A7B8A"; GD = "#D4A574"; MU = "#718096"
IK = "#2D3748"; WH = "#FFFFFF"; BG = "#FAF8F4"
W, H = 1653, 2339
...
def p1_cover():
    ...
    # Large navy block at top
    d.rectangle([0, 0, W, 300], fill=rga(NW))
    d.rectangle([0, 300, W, 310], fill=rga(GD))
    # Decorative circles
    d.ellipse([W-600, -200, W-80, 300], fill=rga(TL, 30))
    d.ellipse([W-480, -120, W-200, 200], fill=rga(GD, 20))
    # Brand
    d.text((140, 80), "SICHUAN", font=font(72, True), fill=rga(WH, 200))
    d.text((140, 155), "RONGCE", font=font(100, True), fill=WH)
    # Tagline with gold accent line
    d.text((140, 380), "谋专业之策  融品质之精", font=font(48, True), fill=NW)
    d.text((140, 460), "政府审计与工程咨询综合服务机构", font=font(30), fill=MU)
    d.line([140, 540, 520, 540], fill=rga(GD), width=5)
    # Bottom info bar
    d.rectangle([0, H-60, W, H], fill=rga(NW))
    d.text((140, H-42), "审计  ·  绩效  ·  财政监督  ·  工程咨询  ·  数字化分析", font=font(22), fill=rga(WH, 200))
    # Corner accents
    d.line([W-90, 80, W-30, 80], fill=rga(GD, 60), width=3)
    d.line([W-30, 80, W-30, 130], fill=rga(GD, 60), width=3)
    # Small decorative diamond at bottom-left
    d.polygon([(50, H-100), (70, H-80), (50, H-60), (30, H-80)], fill=rga(GD, 40))
    d.polygon([(50, H-95), (65, H-80), (50, H-65), (35, H-80)], fill=rga(GD, 60))
    return img

# ======= P2: Why Rongce (improved layout) =======
def p2_about():
    img = Image.new("RGB", (W, H), rgb(WH))
    d = ImageDraw.Draw(img, "RGBA")
    left_bar(d)
    page_title(d, "为什么是融策？", "WHY RONGCE")
    # Visual accent: large faded number
    d.text((W-200, 10), "02", font=font(200, True), fill=rga(NW, 8))
    # Core question
    ...
    for i, (icon, head, desc) in enumerate([
        ...
        x = 160 + i * 380
        d.rounded_rectangle([x, 470, x+340, 680], radius=14, fill=WH, outline=rga(TL, 50), width=2)
        d.rounded_rectangle([x, 470, x+340, 555], radius=14, fill=rga(TL, 25))
    ...
    # Company brief with tighter layout
    y = 730
    d.rounded_rectangle([160, 720, 1490, 860], radius=14, fill=rga("#E8E0D4", 60))
    for item in [
...
# ======= P3: Method (redesigned - more visual) =======
def p3_method():
    img = Image.new("RGB", (W, H), rgb(WH))
    d = ImageDraw.Draw(img, "RGBA")
    left_bar(d)
    page_title(d, "我们怎么做？", "OUR METHODOLOGY")
    d.text((W-200, 10), "03", font=font(200, True), fill=rga(NW, 8))
    # Three lines - horizontal layout with connecting arrows
    for i, (num, title, items) in enumerate([
        ("01", "资金线", ["钱从哪来？到哪去？花得值不值？", "资金流、支付流、票据流三流交叉核验"]),
        ("02", "项目线", ["立项、招标、合同、施工、验收、结算", "全生命周期穿透核查，不留死角"]),
        ("03", "责任线", ["决策程序、岗位职责、内控制度、整改落实", "把问题追溯到人，结论有依据、责任可追溯"]),
    ]):
        x = 160 + i * 380
        d.rounded_rectangle([x, 280, x+340, 570], radius=16, fill=WH, outline=rga(TL, 60), width=2)
        ...
        yy = 400
        for item in items:
            d.ellipse([x+32, yy+6, x+44, yy+18], fill=rga(GD))
            yy = draw(d, item, x+55, yy, font(22), MU, 340-70, 8)
    # Five steps with connecting line
    y0 = 630
    d.line([200, 655, 1450, 655], fill=rga(GD, 60), width=3)
    steps = ["制度审查", "数据核验", "现场核查", "证据闭环", "整改建议"]
    for i, step in enumerate(steps):
        x = 200 + i * 270
        d.ellipse([x, 615, x+80, 695], fill=rga(NW if i%2==0 else TL))
        tw = d.textlength(str(i+1), font=font(32, True))
        d.text((x+40-tw//2, 633), str(i+1), font=font(32, True), fill=WH)
        draw(d, step, x+100, 640, font(26), IK, 150, 6)
    # Core statement - wider card
    d.rounded_rectangle([160, 800, 1490, 920], radius=14, fill=rga("#E8E0D4", 80))
    draw(d, "我们的目标不是出具一份报告，而是帮助委托方看清问题、厘清责任、找到改进路径。", 200, 835, font(28), IK, 1250, 10, "center")
    bottom_bar(d)
    return img

# ======= Service page template (improved visual) =======
def service_page(title, en, sub, items, n):
    img = Image.new("RGB", (W, H), rgb(WH))
    d = ImageDraw.Draw(img, "RGBA")
    left_bar(d)
    page_title(d, title, en, 100)
    # Large faded page number
    d.text((W-200, 30), f"0{n}", font=font(180, True), fill=rga(NW, 8))
    d.text((W-200, 150), f"0{n}", font=font(180, True), fill=rga(TL, 6))
    # Subtitle
    draw(d, sub, 160, 240, font(30), MU, 620, 10)
    # Right decorative box - relocated higher
    d.rounded_rectangle([920, 120, 1450, 500], radius=20, fill=rga(TL, 12), outline=rga(TL, 30), width=2)
    d.text((1000, 170), "0"+str(n), font=font(80, True), fill=rga(TL, 60))
    d.text((1000, 270), "RONGCE", font=font(40, True), fill=rga(TL, 40))
    tags = {
        1: ["从单项到全过程", "从结果到制度"],
        2: ["从有没有", "到值不值"],
        3: ["让资金从花了", "走向花得值"],
    ...
    for i, t in enumerate(tags):
        draw(d, t, 1060, 350+i*60, font(28, True), rga(TL, 80), 400, 6)
    # Service items
    y = 330
    for i, item in enumerate(items):
        clr = rga(NW if i%2==0 else TL)
        d.rounded_rectangle([160, y, 1580, y+48], radius=8, fill=WH, outline=rga(GD, 80), width=1)
    ...
    d.line([160, H-65, 1580, H-65], fill=rga(GD, 30), width=2)
    bottom_bar(d)
    return img
...
# ======= P9: Experience (redesigned) =======
def p9_experience():
    img = Image.new("RGB", (W, H), rgb(WH))
    d = ImageDraw.Draw(img, "RGBA")
    left_bar(d)
    page_title(d, "代表经验", "REPRESENTATIVE EXPERIENCE")
    d.text((W-200, 30), "09", font=font(180, True), fill=rga(NW, 8))
    draw(d, "长期服务省、市、县多级财政和主管部门，覆盖预算绩效、财政监督、专项资金评价、工程决算和管理咨询。", 160, 240, font(30), MU, 620, 10)
    # Right decorative box - service territory
    d.rounded_rectangle([880, 120, 1450, 520], radius=20, fill=rga(TL, 12), outline=rga(TL, 30), width=2)
    d.text((960, 170), "服务版图", font=font(36, True), fill=rga(TL, 60))
    d.text((960, 220), "SERVICE TERRITORY", font=font(18), fill=rga(TL, 40))
    ...
    for i, (nm, dc) in enumerate(terr):
        ...
        if i < 3: d.line([975, y0+30, 975, y0+58], fill=rga(GD, 100), width=2)
    # Representative clients - grid layout
    d.text((160, 380), "代表客户", font=font(32, True), fill=NW)
    d.text((160, 418), "REPRESENTATIVE CLIENTS", font=font(18), fill=rga(GD))
    ...
    for i, c in enumerate(clients):
        ...
        cx = 160 + (i%5)*280
        d.rounded_rectangle([cx, yv, cx+240, yv+36], radius=6, fill=WH, outline=rga(GD, 80), width=1)
        tw = d.textlength(c, font=font(20))
        d.text((cx+120-tw//2, yv+5), c, font=font(20), fill=NW)
    d.text((160, 700), "覆盖领域：财政、审计、教育、民政、交通、国资、医保、药监、公安", font=font(22), fill=MU)
    d.line([160, H-65, 1580, H-65], fill=rga(GD, 30), width=2)
    bottom_bar(d)
    return img

# ======= P10: Contact (redesigned - full navy) =======
def p10_contact():
    img = Image.new("RGB", (W, H), rgb(NW))
    d = ImageDraw.Draw(img, "RGBA")
    # Gradient overlay
    d.rectangle([0, 0, W, H], fill=rga(TL, 15))
    # Top gold accent
    ...
    d.rounded_rectangle([130, 340, 780, 680], radius=16, fill=rga(WH, 30), outline=rga(GD, 80), width=2)
    ...
    d.rounded_rectangle([880, 340, 1530, 680], radius=16, fill=rga(WH, 30), outline=rga(GD, 80), width=2)
...
# ======= PDF generation (200dpi native resolution) =======
def generate_pdf(images):
    # Use A4 at 200dpi: 200 * 8.27 = 1654, 200 * 11.69 = 2338
    pw, ph = 1654, 2338
    c = canvas.Canvas(str(PDF), pagesize=(pw, ph))
    for img_path in images:
        im = Image.open(img_path).convert("RGB")
        # Fit full-page at native resolution
        tmp = WK / "tmp_render_full.jpg"
...
def generate_docx(images):
    ...
    doc = Document()
    doc.core_properties.title = "四川融策宣传册完善稿v6视觉升级版"
