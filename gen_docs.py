...
FH='黑体'; FF='仿宋'; FS='宋体'
SE=Pt(22); SS=Pt(16); SI=Pt(14); SW=Pt(10.5)


def mods(t, d):
    if d=='02':
        t=t.replace('D等比例不低于5%（强制分布，避免老好人现象）','D等比例参考值为5%左右（指导性比例，非强制分布。如全员达标可无D等）')
        t=t.replace('360度协作评分','部门负责人协作评分').replace('360度评价','部门负责人评价').replace('360度评分','部门负责人评分')
        t=t.replace('| 季度考核 | 每季度一次 | 季度结束后10个工作日内完成 | 绩效工资清算 |','| 半年度考核 | 每半年一次 | 考核期结束后5个工作日内完成 | 绩效工资清算 |')
        t=t.replace('季度考核','半年度考核').replace('连续两个季度考核为D等','连续两次半年度考核为D等').replace('连续两次D','连续两次考核D等')
        ...
        t=t.replace('**第四十五条** 公司不监控员工个人通讯内容。因业务需要，公司可以对工作邮箱、工作电脑中的工作文件进行合理管理，但应事先告知员工。','**第四十五条** 公司不监控员工个人通讯内容。因保护商业秘密和信息安全的需要，公司可对工作邮箱和工作电脑中的工作文件进行合理管理。目的限于保护客户信息安全、防止商业秘密泄露。范围限于工作文件，不涉及个人通讯。应事先书面告知员工。')
        ...
        t=t.replace('扣减当年全部绩效工资，降级使用','扣减绩效工资（每月不超过当月工资20%），降级使用')
        t=t.replace('扣减当年50%绩效工资','扣减绩效工资（每月不超过当月工资20%）')
        t=t.replace('扣减当季绩效工资的50%~100%','扣减绩效工资（每月不超过当月工资20%）')
        t=t.replace('扣减当季绩效工资的30%','扣减绩效工资（每月不超过当月工资15%）')
        t=t.replace('扣减当季绩效工资的20%','扣减绩效工资（每月不超过当月工资10%）')
    elif d=='08':
        t=t.replace('扣减当年全部绩效工资，降级使用','扣减绩效工资（每月不超过当月工资20%），降级使用')
        t=t.replace('扣减当年50%绩效工资','扣减绩效工资（每月不超过当月工资20%）')
        t=t.replace('扣减当季绩效工资的50%~100%','扣减绩效工资（每月不超过当月工资20%）')
        ...
        t=t.replace('- 基本合格：可延长试用期（累计不超过法定上限），明确改进目标；','- 基本合格：明确改进目标，给予额外辅导；仍不符合要求的，在试用期内依法解除；')
    elif d=='13':
        t=t.replace('5. **形成坏账的**：对应项目奖金不予发放，已发放的予以追回。','5. **形成坏账的**：对应项目奖金尚未发放的不再发放；已经发放的不予追回。')
    elif d=='22':
        t=t.replace('扣减全年绩效奖金100%','扣减绩效工资（每月不超过当月工资20%）')
        t=t.replace('扣减全年绩效奖金50%以上','扣减绩效工资（每月不超过当月工资20%）')
        t=t.replace('扣减全年绩效奖金30%~50%','扣减绩效工资（每月不超过当月工资15%）')
        t=t.replace('扣减季度绩效奖金20%~30%','扣减绩效工资（每月不超过当月工资10%）')
        t=t.replace('扣减全年绩效奖金50%','扣减绩效工资（每月不超过当月工资20%）')
        t=t.replace('③降级（降低岗位等级）','③调岗或降级（须经员工协商一致）')
        t=t.replace('③降级或调岗','③调岗或降级（须经员工协商一致）')
        ...
        t=t.replace('1. **增量预算法**：以历史数据为基础，结合业务增长预期进行调整（适用于稳定业务）；\n2. **零基预算法**：不参考历史数据，逐项论证支出必要性（适用于新项目、新业务）；\n3. **弹性预算法**：设定不同业务量情景下的预算方案（适用于收入不确定性较大的情况）。','采用**增量预算法**：以上年度实际收支为基础，结合业务增长预期调整。公式：新年度预算=上年度实际数×（1±调整比例）。调整比例由财务部建议，管理层审定。')
    elif d=='04':
