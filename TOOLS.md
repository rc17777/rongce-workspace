...
> 2026-06-01 制定，平头哥要求：每篇入库文章必须含"发现问题的逻辑链"

### 入库必含内容

每篇入库的案例/文章必须包含以下模块：

1. **Frontmatter 元数据**：created / source / url / type / audit_type / tags
2. **正文结构化**：按原文逻辑结构拆分，保留核心判断点
3. **与融策知识库关联**：标明该条目与已有规则/案例/工具的交集
4. **🔗 发现问题逻辑链（必填）**：
...
| 类型 | 适用场景 | 示例 |
|------|---------|------|
| **数据驱动型** | 有大量数据，从异常反推行为 | CASE-DATA-VOUCHER（消费券五层漏斗） |
| **制度对照型** | 有明确法规/制度标准，逐条对照 | CASE-MED-ECA（医疗经责20环节） |
| **归纳总结型** | 从大量项目经验聚类提炼共性 | CASE-ECA-14Q（14类问题归因） |
| **清单遍历型** | 审计范围确定，逐个检查点排查 | CASE-PROJ-75（75项必查） |
| **深度推断型** | 单个规则/现象的深度拆解 | P005/P012 深度分析 |

### 入库检查清单

- [ ] 来源完整（公众号/作者/URL/发布日期）
- [ ] 正文结构化（非原文搬运，而是提炼要点+对照表）
- [ ] 与现有知识库交叉引用
- [ ] 逻辑链 ✅
- [ ] sources/index.md 已更新
- [ ] 主 index.md 案例/规则/行业索引已更新
