...
## 🔍 OCR 进程监控（临时任务：若尔盖医保 OCR，2026-08-01 起）

**每 2 小时检查一次**（09:00/11:00/13:00/15:00/17:00/19:00/21:00/23:00）：

```bash
python -X utf8 C:\Users\scrccpa\.openclaw\workspace\temp_ocr\_check_progress.py
```
...
| 情况 | 动作 |
|:--|:--|
| 无 python 进程 或 全部批次 DONE | 推送微信：「OCR 已全部完成/进程已退出」 |
| 有进程但进度 2 小时没变 | ⚠️ 推送微信：「OCR 疑似卡死，进度 X 小时未更新」 |
| 有进程正常推进 | 静默（不打扰） |
