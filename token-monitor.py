...
def get_token_usage(target_date):
    ...
    for jsonl_file in AGENTS_DIR.rglob("*.jsonl"):
        ...
        try:
            with open(
                jsonl_file, "r", encoding="utf-8", errors="ignore"
            ) as f:
                for line in f:
                    ...
                    results[key]["input"] += (
                        int(input_match.group(1)) if input_match else 0
                    )
                    results[key]["output"] += (
                        int(output_match.group(1)) if output_match else 0
                    )
                    results[key]["cache_read"] += (
                        int(cache_read_match.group(1))
                        if cache_read_match
                        else 0
                    )
                    results[key]["cache_write"] += (
                        int(cache_write_match.group(1))
...
def write_daily_report(target_date):
    ...
    if total_tokens > ALERT_THRESHOLD:
        report_lines.extend(
            [
                "[ALERT] **异常提醒**: 昨日总消耗超过 500K tokens，建议检查是否有异常调用",
                "",
            ]
        )
    ...
    with open(LOG_FILE, "a", encoding="utf-8") as f:
        f.write(
            f"{target_date} | Total: {total_tokens:,} tokens | Calls: {total_calls}\n"
        )

    print(f"[OK] 报告已保存: {REPORT_FILE}")
    print(f"[STATS] 总消耗: {total_tokens:,} tokens ({total_calls} 次调用)")
